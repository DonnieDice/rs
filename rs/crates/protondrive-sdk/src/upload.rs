...
pub async fn upload<R: AsyncRead + Unpin + Send>(
    ...
    // Phase 2 deliverable. Skeleton reads and discards the stream to
    // validate the API signature compiles correctly end-to-end.
    ...
    Err(protondrive_core::error::DriveError::Auth(
        "upload not yet implemented (Phase 2)".into(),
    ))
}
